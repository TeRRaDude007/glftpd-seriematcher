...
##################################################
### TeRRaDuDe SERiE MATCHER on DIR TAGS v3.0 #####
##################################################
#
...
# 3.0 Added HLG tag to check next to HDR
#
#################################################
#############    CONFIG SETUP    ################
#################################################
...
# Ensure the HDR/HLG destination directory exists
...
for dir in "$SRC_DIR"/*; do
    ...
    # Check if the directory contains .HDR or .HLG
    if [[ "$dir_name" == *".HDR"* || "$dir_name" == *".HLG"* ]]; then
        hdr_hlg_dirs["$base_name"]="$dir_name"
    else
...
for base_name in "${!hdr_hlg_dirs[@]}"; do
    if [[ -n "${non_hdr_dirs[$base_name]}" ]]; then
        # If a matching HDR/HLG directory exists for this non-HDR base name, prepare to move the non-HDR directory
        echo "Match found for $base_name with HDR/HLG in one directory:"
        echo " - Non-HDR directory: ${non_hdr_dirs[$base_name]}"
        echo " - HDR/HLG directory: ${hdr_hlg_dirs[$base_name]}"
...
if [ ${#to_move_list[@]} -gt 0 ]; then
    echo
    echo "The following non-HDR/non-HLG directories are ready to be moved to $DEST_DIR:"
    for non_hdr_dir in "${to_move_list[@]}"; do
        echo " - $non_hdr_dir"
    done
    echo
    read -p "Do you want to move all these non-HDR/non-HLG directories? (y/n): " confirm_all
    if [[ "$confirm_all" == "y" ]]; then
        for non_hdr_dir in "${to_move_list[@]}"; do
            echo "Moving non-HDR/non-HLG directory: $non_hdr_dir"
            mv "$SRC_DIR/$non_hdr_dir" "$DEST_DIR"
        done
        echo "All selected non-HDR/non-HLG directories have been moved."
    else
